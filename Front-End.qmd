  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          box(width = 12, div(strong("Top Customer"),style="text-align: center;font-size: 150%"))
          ),
        fluidRow(
          column(12, dataTableOutput("top_customer_table"))
          )
      ),
...
output$age_hist <- renderHighchart({
  ...
  top_customer_query <- "SELECT CustomerID, 
  ...
  # Render DataTable
  output$top_customer_table <- renderDataTable({
    dbGetQuery(con_db, top_customer_query)
  })
